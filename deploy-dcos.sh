...
set -ex
PUBLIC_IP="$(./get-public-ip.sh)"
echo "$PUBLIC_IP"
echo $(echo $PUBLIC_IP | tr -d '\r')
PUBLIC_AGENT_NAME="$(nslookup "$(echo $PUBLIC_IP | tr -d '\r')" | grep -o -e 'name = .*.$' | cut -d '=' -f2)"
PUBLIC_AGENT="${PUBLIC_AGENT_NAME:1:${#PUBLIC_AGENT_NAME}-2}"

if [ -z "$PUBLIC_AGENT" ]; then
   echo "Could not get public agent, aborting"
   exit 1
fi
  ...
  "apps": [
    {
      ...
      "container": {
        ...
        "docker": {
          "forcePullImage": true,
          "image": "danielmschmidt/graphql-performance-tracing-backend",
          "parameters": [],
          "privileged": false
        },
        "volumes": [],
      ...
      "disk": 0,
      "env": {
        "ZIPKIN_HOST_PORT": "http://zipkin.marathon.l4lb.thisdcos.directory:9411",
        "USERS_ENDPOINT": "http://spread.marathon.l4lb.thisdcos.directory",
        "USER_ENDPOINT": "http://id.marathon.l4lb.thisdcos.directory"
      },
      "executor": "",
      "healthChecks": [
        {
...
sleep 3
# Open cluster
dcos cluster list --attached --json | jq ".[].url" | xargs open

# Open public agent
open "http://$PUBLIC_AGENT"

# Open zipkin
open "http://$PUBLIC_AGENT:9411"